...
def upload_to_salsify(con, connection_string):
    ...
    try:
        ...
        # Step 1: Read all parquet data
        progress_container.info("Reading parquet data...")
        df = con.execute("""
        ...
        progress_container.info("Converting to Excel format...")
        ...
        try:
            blob_client = container_client.get_blob_client("salsify-sftp/salsify.xlsx")
            ...
            history_blob_client = container_client.get_blob_client(
                f"salsify-sftp/history/salsify-{timestamp}.xlsx"
            )
        ...
        # Show summary
        st.metric("Records Uploaded", len(df))
...
def show_salsify_upload(con):
    ...
    st.markdown("""
    This tool will:
    1. Create a backup of the existing Salsify Excel file
    2. Generate a new Excel file from the current PIG data
    3. Upload the new Excel file to the Salsify SFTP location
    """)
